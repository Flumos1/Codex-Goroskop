...
for (let byte = 0; byte < 128; byte += 1) {
  cp1251ByteByChar.set(String.fromCharCode(byte), byte);
}
for (let i = 0; i < cp1251Chars.length; i += 1) {
  cp1251ByteByChar.set(cp1251Chars[i], i + 128);
}

function looksMojibake(value) {
  return /Р[А-Яа-яЁёІЇЄЎЌЋЏЃЂ]|С[А-Яа-яЁёІЇЄЎЌЋЏЃЂ]|вЂ|В°/.test(String(value || ""));
}

function repairMojibake(value) {
  const text = String(value ?? "");
  if (!looksMojibake(text)) return text.replace(/В°/g, "°");
  const bytes = [];
  for (const ch of text) {
    bytes.push(cp1251ByteByChar.get(ch) ?? ch.charCodeAt(0));
  }
  const repaired = Buffer.from(bytes).toString("utf8").replace(/�/g, "");
