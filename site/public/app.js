...
// Interpretation texts matched against the full rule set, ranked by
// significance. profile.matchedRules is attached server-side by lib/astro.cjs.
function renderFactors(profile) {
  const matched = Array.isArray(profile.matchedRules) ? profile.matchedRules : [];
  const list = $("#factors-list");

  if (!matched.length) {
    list.innerHTML = `<div class="empty-state">Для этой карты не нашлось трактовок.</div>`;
    $("#factor-systems").innerHTML = "";
    return;
  }

  const counts = new Map();
  matched.forEach((item) => {
    const system = (item.rule && item.rule.system) || "western";
    counts.set(system, (counts.get(system) || 0) + 1);
  });
  $("#factor-systems").innerHTML = [...counts.entries()]
    .map(([system, count]) => `<span class="stat-chip">${escapeHtml(SYSTEM_LABELS[system] || system)}: ${count}</span>`)
    .join("");

  list.innerHTML = matched.map((item) => {
    const rule = item.rule || {};
    const simple = rule.simpleRu || rule.simple || {};
    const advanced = rule.advancedRu || rule.advanced || {};
    const themes = (rule.themesRu || rule.themes || []).slice(0, 5);
    const body = [simple.summary, simple.pattern, simple.growth]
      .filter(Boolean)
      .map((text) => `<p>${escapeHtml(text)}</p>`)
      .join("");
    const technical = [advanced.technical, advanced.method].filter(Boolean).join(" ");

    return `
      <article class="factor-card">
        <header class="factor-head">
          <strong>${escapeHtml(item.query || rule.id || "")}</strong>
          <span class="aspect-badge">${escapeHtml(SYSTEM_LABELS[rule.system] || rule.system || "")}</span>
        </header>
        ${themes.length ? `<div class="factor-themes">${themes.map((theme) => `<span class="factor-theme">${escapeHtml(theme)}</span>`).join("")}</div>` : ""}
        <div class="factor-body">${body || `<p class="empty-state">Текст трактовки отсутствует.</p>`}</div>
        ${technical ? `<details class="factor-detail"><summary>Технический разбор</summary><p>${escapeHtml(technical)}</p></details>` : ""}
        <div class="aspect-meta">Достоверность: ${escapeHtml(rule.confidence || "не указана")}</div>
      </article>
    `;
  }).join("");
}
